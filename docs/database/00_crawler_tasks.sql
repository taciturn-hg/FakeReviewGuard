CREATE TABLE `00_crawler_tasks` (
    ...
    `status` TINYINT DEFAULT 0 COMMENT '任务状态: 0-等待中, 1-进行中, 2-已完成, 3-失败(含登录等待超时/数据库异常等), 4-等待手动操作(如登录), 5-已停止(用户主动停止)',
    `resume_signal` TINYINT DEFAULT 0 COMMENT '恢复信号: 0-暂停/无动作, 1-恢复执行(用于登录后的手动恢复), 2-停止爬虫(用户主动停止)',
