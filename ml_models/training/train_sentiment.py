...
def train_fake_review_detector():
    ...
    if not os.path.exists(input_file):
        print(f"Error: {input_file} not found. Please ensure data is available.")
        ...
        if os.path.exists('labeled_reviews.csv'):
            ...
            print(f"Found {input_file} in current directory, using it.")
        else:
            return
    ...
    if 'label' not in df.columns:
        print("Error: 'label' column missing in data.")
        return
    ...
    if len(df['label'].unique()) < 2:
        print("Error: Need both 'Real' and 'Fake' labels to train a classifier.")
        return
    ...
    print("Extracting sentiment features (using SnowNLP)...")
    ...
    print("Training model with SGD (monitoring loss)...")
